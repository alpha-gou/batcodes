...
class BaseLLM:
    ...
    def generate_async(
        ...
        # print(len(prompt_token_ids), self.args.max_seq_len)
        if self.args.max_seq_len <= len(prompt_token_ids):
            raise Exception(f"Input token length exceeds max sequence length, {len(prompt_token_ids)} >= {self.args.max_seq_len}")
